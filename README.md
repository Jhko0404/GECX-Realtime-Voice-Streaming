# Google Cloud GECX Real-Time Voice Streaming & Telemetry Console
...
### Problem Statement
1. **Sub-Second 저지연 음성 대화 (0.3~0.5s)**:
   * 기존 STT -> LLM -> TTS 3단계 직렬 파이프라인의 지연(2~4초)을 제거하고, Gemini 기반 **Audio-to-Audio (A2A) Native Audio** 실시간 스트리밍을 제공합니다.
2. **80~120초 소켓 세션 단절 원인 실증 분석 (RCA Telemetry)**:
   * 현업 환경에서 발생하는 80~120초 소켓 단절 현상에 대해 **RFC 6455 Close Code(1006, 1007 등) 및 마이크로초 단위 텔레메트리 로그를 수집하여 5대 가설(Server Timeout / Proxy Idle / Silence VAD 등)을 검증**합니다.
3. **엔터프라이즈 제어/데이터 플레인 분리 보안 아키텍처**:
   * **Google Cloud API Gateway**를 통해 세션 발급(OIDC)을 수행하고, **비공개 Cloud Run BFF**와 단기 서명 티켓(JWT, 60s TTL)으로 WebSocket을 직접 연결합니다.
...
```mermaid
flowchart TB
    subgraph ClientLayer["1. Web Client (Frontend Cockpit)"]
        direction TB
...
| :--- | :--- |
| **2열 스플릿 콕핏 UI** | Linear 다크 테크 스타일의 고밀도 엔지니어링 대시보드 인터페이스 제공. |
...
| **단절 RCA 진단 모달** | 80~120초 단절 발생 시 RFC 6455 Close Code 및 5대 가설 매핑 결과 표출, JSON 리포트 다운로드. |
| **오프라인 Mock GECX** | GCP 연결 없이 로컬에서 90초/120초 단절 시뮬레이션 및 테스트 지원 (`--mock 90`). |

---

## 4. Audio DSP & Streaming Specifications
...
## 5. 80~120s Disconnection RCA & 5-Hypothesis Resolution Matrix
...
| **H3: 턴(Turn) 불일치 (1007)** | 에이전트 응답 수신 중 사용자 오디오 잘못 전송 | **Turn-Gated State Machine (`USER_TURN` <-> `AGENT_TURN`)**으로 제어 |
...
## 6. Deployment & Quickstart Guide

### 방법 1: Claude Code를 통한 자동 배포 (권장)
AI 코딩 에이전트 **Claude Code**를 활용하여 명령어 한 줄로 전체 인프라(GCP API, IAM, Cloud Run BFF, API Gateway)를 프로비저닝합니다.
...
### 로컬 오프라인 Mock 모드 & 시연 (Offline Simulation)
GCP 권한이나 네트워크 연결 없이 로컬에서 전체 UI와 90초/120초 세션 단절 RCA 진단 모달을 시연할 수 있습니다.

```bash
# [모드 A] 로컬 90초 단절 시뮬레이션 Mock 모드 실행
./scripts/run_local.sh --mock 90

# [모드 B] 실제 GCP GECX(ces.googleapis.com) 연동 모드 실행
./scripts/run_local.sh
```
* 브라우저에서 `http://localhost:8080` 접속 -> `[CONNECT & START SESSION]` 클릭
* 마이크 실시간 음성 발화 테스트 진행

---

### 10분 연속 스트리밍 부하/진단 테스트 (Automated RCA Runner)
```bash
# 10분간 50ms 오디오 청크를 연속 스트리밍하여 단절 시점과 Close Code 측정
...
## 8. Master Troubleshooting Matrix
